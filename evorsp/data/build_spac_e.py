...
SPLIT = {"train": [f"t{i}" for i in range(1, 9)],
         "val": ["a1", "a2", "a3", "a4"],
         "test": ["b1", "b2", "b3", "b4"]}
...
def main():
    ...
    for seq in sorted(os.listdir(MERGE)):
        ...
        if sp is None:
            continue
        gtdir = f"{GT}/{seq.split('_')[0]}_GT"   # t1_Rain_02 -> t1_GT
        for p in sorted(glob.glob(f"{MERGE}/{seq}/*.npz")):
            g = f"{gtdir}/{os.path.basename(p)}"
            if os.path.exists(g):
                jobs.append((p, g, f"{OUT}/{sp}/{seq}/{os.path.basename(p)}"))
    print(f"{len(jobs)} frames", flush=True)
    with Pool(2) as pool:   # 4 CPUs, four GPU jobs already running
        done = 0
