...
# n_extra is a property of the CHECKPOINT, not a constant. The fair-harness
# head arms were trained with --ctx 0, so their trunks take 10 input channels
# (5 frontend bands x 2 polarities) where a ctx-2 trunk takes 14. Hardcoding 4
# here silently fails to load them.
N_EXTRA_H = hck["trunk"]["in_proj.0.weight"].shape[0] - 10
trunk_h = ORSPNet3D(T=4, num_blocks=3, use_off=True, dilations=(1, 8, 32, 64),
                    out_chans=16, n_extra=N_EXTRA_H)
...
with torch.no_grad():
    for mm in ("50mm", "150mm"):
        for f in sorted(glob.glob(f"{SRC}/merge_data/{mm}/*.npz"))[::8]:
            ...
            logit_map = trunk_h(on4, x_off=off4,
                                x_extra=ex if N_EXTRA_H else None)
            fmap = feats["f"]
