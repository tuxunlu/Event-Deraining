"""EvORSP-3T/E: same trunk, event-accounting supervision. KITTI protocol.
...
def main():
    ...
    tag = f"k3de_T{a.tfront}o{a.tout}_{a.target}" + (f"_s{a.seed}" if a.seed else "")
    ...
    print(f"  TEST MEAN EVENT-DA {test:.4f}   (val tau {best_tau:.2f}, "
          f"val {best:.4f})")
    print(f"  reference: PRE-Mamba 0.9172 | old target ceiling 0.6981 | "
          f"this target's ceiling {'0.9440' if a.target == 'maj' else '0.8789'}"
          f" at T_out=1")
    print(f"  wall {(time.time()-t0)/60:.0f} min")
    torch.save({"state_dict": best_sd, "tau": best_tau, "test": test},
               f"{TMP}/{tag}.pt")
    json.dump({"tag": tag, "params": npar, "tout": a.tout, "target": a.target,
