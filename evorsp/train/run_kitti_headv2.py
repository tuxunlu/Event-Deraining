...
def main():
    ...
    tag = "phv2" + (f"_s{a.seed}" if a.seed else "")
    ...
    trunk = ORSPNet3D(T=4, dilations=(1, 8, 32, 64), num_blocks=3,
                      use_off=True, out_chans=16, n_extra=4).to(DEV)
    ...
    def fwd(on, off, ex, xs, ys, tn, patch, tcols):
        lm = trunk(on.to(DEV), x_off=off.to(DEV), x_extra=ex.to(DEV))
        fm = feats["f"]
        To = lm.shape[1]
    ...
    print(f"  refs: trunk 0.9332 | head v1 0.9396 | PRE-Mamba 0.9172")
