...
PACK = f"{C.REAL_PACK}"
CACHE = f"{C.REAL_HEAD}"
ITI = f"{C.REAL_ITI}"   # 8 ITI-regularity cols
SRC = f"{C.REAL_SRC}"
TMP = f"{C.CKPT}"
DEV = "cuda"
...
class CacheSet(Dataset):
    ...
    def __getitem__(self, i):
        ...
        with np.load(cf.replace(CACHE, ITI)) as d2:
        ...
        sc = cf.split("/")[-3]
        with np.load(f"{SRC}/{sc}/merge_data/{mm}/{base}") as d:
...
class HeadV2(nn.Module):
    def __init__(self, feat_dim=32, hidden=64):
        super().__init__()
        din = 1 + feat_dim + PATCH_D + 19 + 8 + 1   # +8 ITI cols
...
def main():
    ...
    tag = f"realiti_{a.split}" + (f"_s{a.seed}" if a.seed else "")
    ...
    print(f"  refs: head-only theirs 0.8444 / ours 0.8686 | PRE-Mamba 0.7708")
