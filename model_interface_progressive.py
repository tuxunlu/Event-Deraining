...
class ModelInterfaceProgressive(pl.LightningModule):
    ...
    def __configure_loss(self):
        ...
        def loss_func(derained, gt, rainy, stage: str):
            ...
            # Background Identity Loss
            bg_mask = (rainy - gt).abs() < 0.1  # Example threshold for background
            bg_diff = (derained - gt).abs()
            ...
            # Rain Loss
            rain_mask = ~bg_mask
            rain_diff = (derained - gt).abs()
            rain_loss = (rain_mask * rain_diff).sum() / (rain_mask.sum() + 1e-8)
            self.log(f'{stage}_rain_loss', rain_loss,
                    on_step=True, on_epoch=True, prog_bar=True, sync_dist=True)
            

            # 4. Final loss
            final_loss = spatial_loss + amp_loss + phase_l + bg_loss + rain_loss
